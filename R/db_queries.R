...
#' RINGS_FOR_RADO
#' @export
#' @param seasons           season(s) from which the data is required. Can be vector or number. Note that this refers to the season, not to the calendar year (e.g. 2018 is summer 2017 through spring 2018)
#' @param new_ring          date and time when a bird recieved a new metal band. Only necessary if there is any chance that the bird was caught in between somewhere else
...
RINGS_FOR_RADO = function(
  ...
  #export to excel; this automatically creates the different sheets required by Radolfszell!
  {
	...
	return(p) #return temporary path in order to find the file
}
